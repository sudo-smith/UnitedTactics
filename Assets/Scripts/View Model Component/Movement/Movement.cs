...
{
    public int maxRange;
    public int rangeLeft; //todo use this for the find tiles in range function
    ...
    {
        return (from.distance + 1) <= rangeLeft;
    }

    protected virtual void Filter(List<Tile> tiles)
    {
        GameObject tileOccupant;
        for (int i = tiles.Count - 1; i >= 0; --i)
        {
            tileOccupant = tiles[i].content;
            if(tileOccupant != null)
            {
                if (tileOccupant.tag == "Enemy")
                    tiles.RemoveAt(i);
            }
        }
